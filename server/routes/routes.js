// const { checkLoginInput, adminEmail, apiUrl } = require("../utils");
var _ = require("lodash");
require("dotenv").config();
const { createJSONToken, validateJSONToken } = require("../auth");
const { getRecipient } = require("../controller/controller");
const md5 = require("md5");
const router = require("express").Router();
const User = require("../schema/user"); // Adjust the path as needed
const { getRoleBasedOnEmail } = require("../utils");
...
const create_user = async (req, res) => {
  try {
    const newCreatedUser = new User({
      ...
      applied: [],
      role: getRoleBasedOnEmail(req.body.email), // Assign role based on email
    });
